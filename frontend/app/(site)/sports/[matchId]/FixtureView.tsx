...
import type { Fixture, SourceReading } from "@/lib/hive/types";
      ...
      <Card className="space-y-4">
        ...
        {f.status === "SETTLED" && (
          <div className="space-y-1">
            ...
            {f.refund_all && (
              <p className="text-sm text-ink/75">Nobody backed {f.result === "DRAW" ? "the draw" : labels[f.result as keyof typeof labels]}, so there are no winners: every stake is refunded 1:1.</p>
            )}
          </div>
        )}
        ...
        <Card className="space-y-4 lg:col-span-2">
          ...
          {position?.exists && (
            <div className="rounded-md border border-black/10 p-3 text-sm">
              ...
              {position.claimed && <> · {f.refund_all ? "refunded" : "claimed"} {formatGen(position.payout)} GEN</>}
            </div>
          )}
          ...
          {f.status === "OPEN" && (
            <div className="space-y-2">
              <TxDialog
                ...
                variant="blue"
                className="w-full"
                disabled={phase !== "READY_TO_SETTLE"}
                ...
                label={phase === "READY_TO_SETTLE" ? "Resolve" : `Resolve available in ${formatCountdown(Math.max(0, f.kickoff_ts + 5400 - now))}`}
              />
              <TxDialog
                address={HIVE_SPORTS_ADDRESS}
