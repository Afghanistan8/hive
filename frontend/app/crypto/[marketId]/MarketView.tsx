...
export function MarketView({ marketId, initial }: { marketId: string; initial: Snapshot<CryptoMarket> | null }) {
        ...
        <Card className="space-y-4 lg:col-span-2">
          ...
          {position?.exists && (
            <div className="rounded-md border border-black/10 p-3 text-sm">
              ...
              {position.claimed && <> · {m.refund_all ? "refunded" : "claimed"} {formatGen(position.payout)} GEN</>}
            </div>
          )}
          {m.state !== "PENDING" && position?.exists && !position.claimed && (
            claimable > 0n ? (
              <ClaimDialog
                ...
                args={[m.id]}
                amount={claimable}
                label={`${m.refund_all ? "Refund" : "Claim"} ${formatGen(claimable)} GEN`}
              />
            ) : (
        ...
        <Card className="space-y-4">
          ...
          {m.state === "PENDING" && (
            <TxDialog
              ...
              label={phase === "READY_TO_SETTLE" ? "Resolve" : `Resolvable in ${formatCountdown(Math.max(0, m.settles_at - now))}`}
            />
          )}
          {evidence?.exists && (
